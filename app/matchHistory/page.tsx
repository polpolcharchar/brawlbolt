...
export default function MatchHistoryOverviewPage() {
  ...
  useEffect(() => {
    if (activePlayerTag) {
      router.push(`/matchHistory/${activePlayerTag}`)
    }
  }, []);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[90vh] px-4 py-12 space-y-10">
      <div className="max-w-4xl text-center space-y-4">
        <h1 className="text-4xl font-bold tracking-tight text-(--foreground)">BrawlBolt's Match History</h1>
        <p className="text-lg text-(--muted-foreground)">
          Every Game You Play - Saved
        </p>
      </div>
      ...
      <div className="grid gap-8 max-w-xl w-full md:grid-cols-1">
        <Card>
          <CardHeader>
            <CardTitle className="text-(--foreground)">View Your Games</CardTitle>
            <CardDescription>
          ...
          <CardContent className="flex flex-col gap-4">
            <PlayerSelector />
            <Button
              onClick={() => router.push(`/matchHistory/${activePlayerTag}`)}
