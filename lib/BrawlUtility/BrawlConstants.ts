...
export let brawlerModeLabelsUpdateAttempted = false;
export const setBrawlerModeLabelsUpdateAttempted = (value: boolean) => {
  brawlerModeLabelsUpdateAttempted = value;
}
...
export const setModeLabels = (labels: { value: string, label: string }[]) => {
  modeLabels = labels;

  modeLabelMap = {};
  for (const entry of labels) {
    modeLabelMap[entry.value] = entry.label
  }

  console.log("Updated modeLabelMap:", modeLabelMap);
}
export let modeLabels = [
  ...
  { "value": "subwayRun", "label": "Subway Run" },
]
export let modeLabelMap: any = {
